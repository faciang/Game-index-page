(async()=>{
  ...
  const loadCoverImage=(b64,key)=>new Promise((resolve,reject)=>{
    const img=new Image();
    img.onload=()=>resolve(img);
    img.onerror=()=>reject(new Error('書封解碼失敗：'+key));
    img.src=`data:${coverMime(b64,key)};base64,${b64}`;
  });
  try{
    if(parts.length!==12)throw new Error(`遊戲程式載入不完整：${parts.length}/12`);
    for(const src of v2Sources)await loadScript(src);
    const missing=expected.filter(k=>!window.BOOK_COVERS?.[k]);
    if(missing.length)throw new Error('缺少書封：'+missing.join('、'));
    for(const key of expected){
      await loadCoverImage(window.BOOK_COVERS[key],key);
    }
    let code=parts.join('');
